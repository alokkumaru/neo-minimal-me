...
const SinglePage = () => {
  const posts = [
    {
      ...
      excerpt:
        "AI workload is driving the necessity for cloud storage services, with cloud storage growing at ~20% CAGR.",
      date: "2025-09-30",
  ...
  const experience = [
    {
      ...
      location: "Bangalore",
      description:
        "Built new consumer hardware products and established R&D at 'Native'. Native is a strategic D2C play to reposition Urban Company as a 'home solutions company' from 'home services' company (NTV USD 18 Mn in FY25, y-o-y growth >300%)",
      highlights: [
    ...
    },
    {
      ...
      location: "Mumbai",
      description:
        "Led 10+ engagements across transaction advisory and strategy with top PE firms, global educational companies, and governments",
      sections: {
        strategyProjects: [
  ...
  return (
    // Top-level container now has the background grid and mask
    <div className="min-h-screen pt-6 md:pt-8 pb-16 md:pb-20 px-5 md:px-6 relative">
      <div className="absolute inset-0 background-grid-light"></div>
      <div className="faint-mask-overlay"></div>

      {/* The main content wrapper needs relative positioning and a higher z-index to sit above the global grid */}
      <div className="max-w-6xl mx-auto bg-background border-neo border-foreground shadow-neo-lg rounded p-6 md:p-8 relative z-10">
        <Navigation />

        {/* Home Section (Removed inner background divs) */}
        <section id="home" className="mb-16 md:mb-24 scroll-mt-24 relative">
          <div className="border-neo border-foreground shadow-neo-lg p-8 md:p-12 rounded mb-8 transform hover:translate-x-2 hover:translate-y-2 hover:shadow-none transition-all mx-[4px] bg-slate-50 relative z-10">
            <h1 className="text-4xl sm:text-5xl md:text-6xl lg:text-8xl font-bold mb-6 md:mb-6 text-primary-foreground">
              Hi, I'm Alok
            </h1>
            <p className="text-lg sm:text-xl md:text-2xl lg:text-3xl text-primary-foreground/90 font-medium mb-8 md:mb-8">
              I'm a consultant turned builder based in Bangalore. Always looking to solve hard, impactful problems.
            </p>
            <div className="flex flex-wrap gap-5 md:gap-6">
              <a
                href="https://x.com/alok_kumar_u"
                target="_blank"
              ...
              <a
                href="https://github.com/alokkumaru"
                target="_blank"
                rel="noopener noreferrer"
                className="hover:opacity-70 transition-opacity"
              >
              ...
              <a
                href="https://www.linkedin.com/in/alok-kumar-u/"
                target="_blank"
                rel="noopener noreferrer"
                className="hover:opacity-70 transition-opacity"
              >
            ...
            {posts.map((post) => (
              <Link key={post.id} to={`/blog/${post.slug}`} className="group cursor-pointer block">
                <article className="bg-card text-foreground border-neo border-foreground shadow-neo rounded p-5 md:p-6 h-full flex flex-col group-hover:translate-x-2 group-hover:translate-y-2 group-hover:shadow-none transition-all">
                  ...
                  <h2 className="text-xl md:text-2xl font-bold mb-3 md:mb-3">{post.title}</h2>

                  <p className="text-sm md:text-sm opacity-90 mb-4 md:mb-4 flex-grow">{post.excerpt}</p>
        ...
        {/* Projects Section (Removed inner background divs) */}
        <section id="projects" className="mb-16 md:mb-24 scroll-mt-24 relative">
          ...
          <div className="bg-card border-neo border-foreground shadow-neo-lg p-8 md:p-12 rounded text-center relative z-10">
            ...
            <p className="text-base sm:text-lg md:text-xl text-muted-foreground">Project showcase coming soon!</p>
          </div>
        </section>

        {/* Journey Section (Removed inner background divs) */}
        <section id="journey" className="scroll-mt-24 relative">
          <div className="text-center mb-6 md:mb-8 relative z-10">
            ...
            <p className="text-lg sm:text-xl md:text-2xl text-muted-foreground px-2">Things I've done</p>
          </div>
                ...
                >
                  <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start mb-3 gap-2">
                    ...
                    <div className="sm:text-right">
                      ...
                      <span className="text-xs md:text-sm text-muted-foreground block sm:inline sm:ml-2">
                        {job.location}
                      </span>
                    </div>
                  </div>
